# ###########################################################################
# this is detecting stairs and shoes, then alert when warning condition is occured
# for detecting stairs, use voting algorithm (A)
# for detecting shoes, use tracking algorithm (B)
# refer to A, open '../stair_case/voting_edge_detection.py'
# refer to B, open '../foot_case/multi_tracking.py'
# ###########################################################################
...
x1, y1, w1, h1 = int(width / 3), int(height * 9 / 10), int(width / 6), int(height / 10)
x2, y2, w2, h2 = int(width * 2 / 3), int(height * 9 / 10), int(width / 6), int(height / 10)
box1 = (x1, y1, w1, h1)
box2 = (x2, y2, w2, h2)
...
# ###########################################################################
def predictNumOfStair(voteStair):
    startPNOS = time.time()
    ...
    for i in range(0, len(voteStair)):
        copyVoteStair.append([voteStair[i], i])
...
def multiTracking(img):
    # if want to get more exactly, add condition for tracking
...
def saveImg():
    ...
    while True:
        ...
        if complete and (len(imgSet) == 0):
            break
        print(complete, len(imgSet))
        img = imgSet.pop(0)
