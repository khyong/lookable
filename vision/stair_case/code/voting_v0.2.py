...
# v0.3 update stairInterval(or num of stairs) when each frame is updated
...
while success:
    ...
    while (preVal != -1) and (seeker < len(stair)):
        check = True
        ...
        if check:
            ...
            if (stair[seeker][0] / preVal) < THRESHOLD:
                preVal = -1
            else:
                preVal = stair[seeker][0]

        seeker += 1

    imgSet.append(img)
    
    cv2.imwrite('../voting_v0.2/test%d.png' %imgCount, img)
...
# write video
outVideo = cv2.VideoWriter('../voting_v0.2.avi', fourcc, 25, (width, height))
